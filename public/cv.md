...
- Clean Architecture y DDD en el frontend: capas `domain / application / infrastructure`
  por módulo, lo que permite test unitario sin montar el framework.
...
`metadata` (Google Books, AniList) → `cli`. El core es completamente testable en aislamiento.
